...
namespace MYnewsWebsite.newsFILE
{
    ...
    public partial class DefenceAndLow : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                SqlDataAdapter da = new SqlDataAdapter("select * from ARTI where catogory_id = 4", con);
                DataTable dt = new DataTable();
                da.Fill(dt);

                //get data length ----------------

                SqlCommand cmd = new SqlCommand("select count(*) from ARTI where catogory_id = 4", con);
                cmd.Connection.Open();
